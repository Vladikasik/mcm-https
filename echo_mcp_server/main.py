...
Examples:
  ...
  # Production mode with environment variables (.env file)
  python -m echo_mcp_server --production
  
  # Production mode with explicit certificates
...
Environment Variables:
  ...
  SSL_CERTFILE        - Path to SSL certificate file (production)
  SSL_KEYFILE         - Path to SSL private key file (production)
  HOST                - Host to bind to (default: 127.0.0.1)
  PORT                - Port to bind to (default: 8443 for HTTPS, 8080 for HTTP)
  SERVER_NAME         - Name for the MCP server (default: EchoHTTPS)
  LOG_LEVEL           - Log level (default: info)
  WORKERS             - Number of worker processes (default: 1)

Production Deployment:
  1. Copy config.env.example to .env
  2. Update SSL_CERTFILE and SSL_KEYFILE paths
  3. Set ENV=production
  4. Run: python -m echo_mcp_server --production --host 0.0.0.0
...
MCP Inspector Usage:
  ...
  Node.js SSL Issues: If MCP Inspector still fails with HTTPS, use:
    ...
    parser.add_argument(
        ...
        help="Host to bind to (default: 127.0.0.1, production: 0.0.0.0)"
    )
    ...
    parser.add_argument(
        "--name",
        default=os.getenv("SERVER_NAME", "EchoHTTPS"),
    ...
    ssl_group.add_argument(
        ...
        help="Path to SSL certificate file (can be set via SSL_CERTFILE env var)"
    )
    
    ssl_group.add_argument(
        "--key-file", 
        default=os.getenv("SSL_KEYFILE"),
        help="Path to SSL private key file (can be set via SSL_KEYFILE env var)"
    )
    ...
    mode_group.add_argument(
        "--production",
        action="store_true", 
        help="Force production mode (requires provided certificates or env vars)"
    )
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    is_production = args.production or env_mode == "production"
    
    # Check SSL certificate files in production mode
    if is_production and not args.no_ssl:
        if not args.cert_file or not args.key_file:
            print("Error: Production mode requires SSL certificates.", file=sys.stderr)
            print("Set SSL_CERTFILE and SSL_KEYFILE environment variables or use --cert-file and --key-file", file=sys.stderr)
            print("Example: Set these in your .env file:", file=sys.stderr)
            print("  SSL_CERTFILE=/path/to/domain.cert.pem", file=sys.stderr)
            print("  SSL_KEYFILE=/path/to/private.key.pem", file=sys.stderr)
            sys.exit(1)
    ...
    # Production mode should bind to 0.0.0.0 by default
    if is_production and args.host == "127.0.0.1" and os.getenv("HOST") is None:
        args.host = "0.0.0.0"
        print("Info: Production mode - binding to 0.0.0.0 (all interfaces)")
    
    # Trust cert only works with development mode
    if args.trust_cert and is_production:
...
def print_mcp_inspector_instructions(no_ssl: bool, host: str, port: int, development_mode: bool, is_production: bool) -> None:
    ...
    if no_ssl:
        ...
        if is_production:
            ...
            print("   Production server with valid SSL certificates")
...
def main():
    ...
    print("="*60)
    mode_display = "Production" if is_production else "Development" if development_mode else "Auto-detect"
    ...
    print_mcp_inspector_instructions(args.no_ssl, args.host, args.port, development_mode, is_production)
    ...
    if args.reload and not is_production:
